...
5. For each applicable item, use `question`, `pass_criteria`, and `failure_signal` as the writing guide. The `suggested_format` field tells you which table template to use (see `04_templates/table-templates/`). **When available, read the JSON execution layer first** (`table-template-index.json` for template routing, `global-component-conventions.json` for field/input/selection/list defaults, `retrospect-trigger-rules.json` for trigger/escalation rules) **then read the Markdown file** for rationale, examples, and boundary rules. JSON provides the mechanical answer; Markdown provides the why.
