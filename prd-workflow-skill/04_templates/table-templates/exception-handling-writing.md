...
> Version: v2.1.0 — adds reachable-state and product-boundary rules
...
- 只写业务主路径上可到达，且会改变用户判断、业务数据状态或恢复动作的异常。
- 纯技术错误、底层接口错误码和过细控件校验，留到页面设计、接口设计或测试用例阶段；除非它们改变产品可见结果。
- 遵循 MECE：一行只描述一个触发条件。只有发生阶段、系统处理、数据状态和提示语完全相同的条件才可合并，并用【】突出差异项。
- 不为理论上存在、但被前置选择范围或流程门禁排除的状态编写页面异常；批量导入等绕过页面选择的入口应单独校验。
- 提示语优先写清“原因 + 立即可执行的下一步”，保持简短；动态字典值、状态值不得硬编码进提示语。
...
| 维度 | 触发示例 | 必查？ |
|------|---------|--------|
| 空态 | 无数据、无结果、字段为空 | 业务流程可到达且需要产品反馈时必查 |
| 失败态 | 提交失败、加载失败、保存失败 | 失败会改变用户操作、数据状态或恢复动作时必查 |
...
| 网络异常 | 请求超时、无响应、部分失败 | 对用户可见结果、数据一致性或恢复路径有影响时必查 |

检查时应记录各维度是否适用；不适用项可在评审轨迹中标记为 N/A，无需在 PRD 正文逐条写“本功能无此异常”。
